...
st.caption("Upload your dataset → Multi-agent pipeline → Gemini AI Insights")
...
def agent_gemini_insights(summary_text, api_key):
...
"""
            ...
            st.markdown("## 🧠 Gemini Insight Agent")
            llm_ph = st.empty()
            llm_ph.markdown('<div class="agent-card agent-running">🔄 <b>🧠 Gemini Insight Agent</b><br><small>Sending results to Gemini for deep analysis…</small></div>', unsafe_allow_html=True)

            insight_box = st.empty()
            full_text = ""
            try:
                for chunk in agent_gemini_insights(summary, api_key):
                    full_text += chunk
                    insight_box.markdown(f'<div class="insight-box">{full_text}▌</div>', unsafe_allow_html=True)
                insight_box.markdown(f'<div class="insight-box">{full_text}</div>', unsafe_allow_html=True)
                llm_ph.markdown('<div class="agent-card agent-done">✅ <b>🧠 Gemini Insight Agent</b><br><small>Insights generated successfully!</small></div>', unsafe_allow_html=True)
            except Exception as e:
                st.error(f"Gemini API error: {e}")
                st.info("Make sure your API key is valid. Get a free key at aistudio.google.com")
    ...
    3. **Gemini AI** synthesises everything into deep insights
    ...
    st.success("🧠 **Gemini Insight Agent** — Gemini AI synthesises all findings into actionable insights")
